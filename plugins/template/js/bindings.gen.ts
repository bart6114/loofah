...
export const commands = {
  async render(tpl: Template): Promise<Result<string, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:template|render", { tpl }),
      };
    } catch (e) {
  ...
  async renderSupport(tpl: SupportTemplate): Promise<Result<string, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:template|render_support", { tpl }),
      };
    } catch (e) {
      if (e instanceof Error) throw e;
      else return { status: "error", error: e as any };
  ...
  async getTemplateSource(
    ...
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("plugin:template|get_template_source", {
          template,
        }),
      };
    } catch (e) {
...
export type ActivityCaptureUser = {
  appName: string;
...
export type DailySummaryUser = {
  date: string;
  timezone: string | null;
  stats: DailySummaryStats;
  topApps: DailySummaryAppStat[];
...
export type Template =
  ...
  | { activityCaptureUser: ActivityCaptureUser }
  | { dailySummarySystem: DailySummarySystem }
  | { dailySummaryUser: DailySummaryUser }
