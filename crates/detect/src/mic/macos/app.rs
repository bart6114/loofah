...
pub(super) fn poll_apps(ctx: &SharedContext) {
    if !ctx.polling_active.load(Ordering::SeqCst) || !ctx.running.load(Ordering::SeqCst) {
        return;
    }
    ...
    let (started, stopped) = diff_apps(&state.active_apps, &current_apps);
    state.active_apps = current_apps;
    drop(state);
    if !started.is_empty() {
        ctx.emit(DetectEvent::MicStarted(started));
    }
    if !stopped.is_empty() {
        ctx.emit(DetectEvent::MicStopped(stopped));
    }
}
