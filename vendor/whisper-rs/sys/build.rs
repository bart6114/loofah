...
fn main() {
    ...
    fs_extra::dir::copy("./whisper.cpp", &out, &Default::default())
        .expect("copy pinned native sources");
...

    {
        ...
        match bindings {
            ...
            Err(e) => panic!("Matching whisper.cpp bindings are required: {e}"),
        }
    };
...
fn get_whisper_cpp_version(whisper_root: &std::path::Path) -> std::io::Result<Option<String>> {
    let source = std::fs::read_to_string(whisper_root.join("CMakeLists.txt"))?;
    let parts: Option<Vec<_>> = ["MAJOR", "MINOR", "PATCH"]
        .iter()
        .map(|part| {
            let prefix = format!("set(WHISPER_VERSION_{part} ");
            source
                .lines()
                .find_map(|line| line.strip_prefix(&prefix).map(|v| v.trim_end_matches(')')))
        })
        .collect();
