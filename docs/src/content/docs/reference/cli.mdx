...
| `sessions rename ID TITLE` | None | Changes only the session title, stored verbatim. Keeps the id, directory, metadata, and content. JSON returns `sessions.rename` with `id` and `title`; missing sessions return `not_found` (exit 2). |
...
| `sessions export ID` | `--format markdown\|md\|json\|pdf\|txt\|org`, `--include note,summary,transcript`, `--output FILE`, `--force` | Export selected content or a legacy complete meeting. Text defaults to stdout; PDF requires a file. See [export behavior](#export-behavior). |
...
loof sessions export ID --format org --include summary
loof --json sessions export ID --format md --include note,summary --output session.md
```

PDF, TXT, and Org default to `note,summary`. `--include` replaces the selection; it accepts comma-separated values or repeated flags. Sections appear in Note, Summary, Transcript order, regardless of flag order. Missing or blank sections are skipped. `summary` includes the session’s single summary.
...
Existing Markdown and JSON calls **without `--include`** keep their complete-session artifact: the note, summary, transcripts, and existing metadata and layout. Their `--json` envelope behavior also stays unchanged, including writing that envelope to `--output` when both flags are used.
