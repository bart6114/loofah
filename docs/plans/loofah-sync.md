...
- [x] Staging artifact, private invitation delivery and two-Mac walkthrough.
...
- QA found and fixed a stale authorization code after cancellation, a missing cached authority immediately after first enrollment, a stale open editor after restore, and a Tantivy related-document panic when term counts included unmerged deletions. The latter has a regression test that failed before the fix; all 20 Tantivy unit tests and its integration test passed afterward. Restore now flushes pending editor writes and reloads the session/editor/audio player; focused tests cover the ordering and failed-write behavior. The corrected signed build from `446a8c0dd` passed its targeted native recheck: cancellation cleared the old approval code, reconnect recovered saved keys and paused state, restore immediately refreshed the open editor, and exporting the preserved revision recovered the new unsynced marker. All 2,008 snapshotted note/registry/excluded files matched their original contents after restore, and the corrected app log contained no related-document panic. The disposable account was disconnected and its Keychain session removed after QA.
- Workspace CI exposed a race between the lock-release test and a concurrently spawning subprocess test. It reproduced locally on the third concurrent run. Serializing those two tests leaves production locking unchanged; all 29 vault-read tests and 100 repeated concurrent transaction-test runs then passed. All CI workflows passed for `5d52f11c8`: formatting/lint, workflow security, CLI, desktop frontend and [desktop/workspace Rust tests](https://github.com/bart6114/loofah/actions/runs/35212941356). The final artifact/QA record is a documentation-only follow-up to that tested commit.

### Installation and two-Mac walkthrough

The current Apple Silicon DMG is available in [workflow 35223975234](https://github.com/bart6114/loofah/actions/runs/35223975234/artifacts/10499142284), built from product commit `000afaa21cdda31c246bf0c2c3840dbab6c4c534`. It includes the Sync UX fixes below. All CI checks and the signed build passed. The artifact expires on 1 October 2026.

- Local DMG: `/Users/bart/Downloads/Loofah-Sync-Staging-000afaa21/Loofah Staging_0.38.0_aarch64.dmg`.
- Installed app: `/Applications/Loofah Staging.app`, identifier `io.loofah.staging`. Normal Loofah remains installed separately. The updated staging app is running with Bart’s existing test vault and account connection preserved.
- SHA-256: `745de2e16062eedf7f761eec6d689baefa36582227450d4f6ecd9940115fb63d`.
- Checksum, strict deep signature, Gatekeeper notarization and stapled-ticket checks passed. Staging has separate settings, Keychain and sync-state directories and no stable updater endpoint.

This remains a draft staging milestone until the outstanding acceptance checks pass. Bart’s private invitation was supplied in the conversation and used successfully; the token is not stored in git. His account and first-Mac enrollment are complete.
...
2. Bart’s account is ready: sign in with `bartsmeets86@gmail.com`. For a new invited account, open its invitation, complete registration and verify the email in the browser. Verification resend and password reset should be tested through the browser; password reset does not restore encryption keys.
3. On the first Mac, open Settings → Sync → Connect and approve its code in the browser. Save the recovery kit outside the vault, then reimport that exact file. Complete **Secure this Mac**, then click **Start syncing** and wait for **Up to date**. Settings shows the signed-in account, selected folder and this Mac; choose a recognizable name under device management.
4. When a second Mac is available, install the same staging build and select a fresh disposable vault. Connect to the same account. Choose either **Import recovery kit…**, or **Pair with a trusted Mac** and enter the displayed code in the trusted Mac’s Sync settings. After pairing, click **Start syncing** on the new Mac. A trusted Mac keeps its existing pause/running preference; use **Resume** if it was paused.
5. Edit a note on each Mac while the other is paused, then resume both. Choose **Review versions**, compare both versions and export copies if needed, then explicitly choose which version to keep and verify both Macs converge. Confirm the recording object is reused for note-only edits.
...
### Account sync overview — 17 September 2026

- Added active synced-item and enrolled-Mac counts, the latest committed cloud change time and a storage usage bar. The allowance now comes from account data. The account page refreshes every 30 seconds while active, and account sign-ins are labeled separately from vault sessions.
- Entity types are hidden by keyed identifiers, so the server cannot accurately separate sessions from the three shared registry records. The page says “Synced items” and explains the scope; deleted heads, historical revisions and conflicting revisions do not inflate that count. Device counts exclude revoked devices. A cloud update timestamp does not claim that every Mac has finished downloading it.
- All 24 cloud tests, Worker runtime verification, workspace TypeScript checks, frontend build and formatting passed. Regression coverage includes empty state, edits, conflicts, deletion, restore, revoked devices and account isolation.
- Bart's desktop history message was checked against Settings → Sync: account login succeeded, but the app was still waiting at “Save your recovery kit.” Saving and reimporting the kit completes the required first-device enrollment before uploads/history become available.
- Deployed staging version `d053bda2-1df7-4d20-954c-a5e8b6869f22` and checked the rendered account page in Chrome: 0 synced items, 0 enrolled Macs and “No changes uploaded yet,” consistent with the pending recovery-kit step.

### Sync UX implementation — 17 September 2026

- Three independent browser, desktop and journey reviews agreed on setup clarity, pairing state, account/device identity and informed conflict decisions as the priorities. Changes use that consensus, with another integration review before delivery.
- Browser setup now shows the signed-in email and an explicit return-to-app handoff. Invitation signup hides the raw token and supports short-lived per-tab refresh recovery; verification success is backed by a signed receipt after token validation. Authentication detours retain the pending Mac code. Missing reset links, offline errors and loading states have distinct next actions. Browser sign-ins identify the current browser and distinguish enrolled Loofah sessions.
- Migration `0010_device_names.sql` adds optional, user-entered Mac names. Renaming requires an active enrolled bearer session and remains scoped to its vault. The account dashboard uses “vault-wide” and “Macs with access” without implying collaboration or online presence.
- Desktop setup presents Sign in → Secure this Mac → Start syncing, preserving explicit first-upload consent. Saved start consent survives restart; previously paused Macs still offer Resume. Browser codes can be reopened/copied. Pairing instructions identify this Mac's role, and pairing no longer changes the trusted Mac's saved pause preference.
- Sync status distinguishes setup, checking, syncing, up to date and needs attention. Up-to-date requires a successful poll in this run, completed reconciliation and no pending work/conflicts. Account usage and devices refresh automatically; names and “This Mac” are visible, with explicit confirmation for revocation.
- History provides setup navigation and an empty state, readable content labels, named origins, selected-version details and specific export/restore/delete results. File-dialog cancellation is neutral. History deletion requires confirmation and clears the removed preview.
