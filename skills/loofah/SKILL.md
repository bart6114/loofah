...
- `transcribe` replaces the existing transcript, so confirm before running it on a meeting that already has one. Both `transcribe` and `import --transcribe` honor audio retention: when the setting is "none", the recording is deleted once the transcript is saved.
