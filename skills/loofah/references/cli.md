...
PDF/TXT/Org default to note and summary. `--include` selects exactly note, summary, and/or transcript; missing sections are skipped. Summary selection uses the first `(sort_order, id)` ascending, including template outputs; pass `--summary-id ID --include summary` to choose another id from `sessions get ID --json`. An explicit missing summary is an error. Sections always appear note, summary, transcript. Dates use local time and CLI labels are English. PDFs embed referenced managed images; text exports do not copy attachments.
...
Export is headless and read-only with respect to the vault; output must be outside it. Writes are atomic. Existing destinations require `--force` (exit 4 otherwise); only pass it after approval for that exact file. Missing session/summary ids return exit 2, and rendering/I/O failures return exit 1. With `--json`, errors go to stderr. Text `--output -` creates a literal file named `-`; omit `--output` for stdout.
