use super::*;
use vault_sync::pairing::PendingPairing;
...
impl Runtime {
    pub(super) async fn start_pairing(&mut self, code: Option<String>) -> Result<(), String> {
        ...
        let authority = account
            ...
            .clone()
            .ok_or("Enroll the first Mac with a saved recovery kit.")?;
        let remote = self.remote.clone().ok_or("Connect your account first.")?;
        if code.is_some() && self.engine.is_none() {
            return Err("Only an enrolled Mac can approve pairing.".into());
        }
        if code.is_none() && self.engine.is_some() {
            return Err("This Mac is already enrolled.".into());
        }
        let credential = tokio::task::block_in_place(|| {
            Keychain::new(Environment::Staging).session(account.account.vault_id)
        })
        ...
        let enrollment_path = self.root.join("pending-enrollment.json");
        self.phase("pairing");
        self.pairing = Some(tokio::spawn(async move {
            tokio::time::timeout(Duration::from_secs(600), async move {
                ...
                if code.is_some() {
                    let (key, _, _) = tokio::task::block_in_place(|| Keychain::new(Environment::Staging).identity(account.account.vault_id)).map_err(|e| e.to_string())?;
                    ...
                    tokio::task::block_in_place(|| { atomic_json(&enrollment_path, &PendingEnrollment { challenge: challenge.clone(), authority_signature: approval.signature.clone() })?; Keychain::new(Environment::Staging).save_confirmed_identity(&key, device, &pair, &approval.kit)?; atomic_json(&directory.join("paired-checkpoint.json"), &approval.checkpoint) }).map_err(|e| e.to_string())?;
